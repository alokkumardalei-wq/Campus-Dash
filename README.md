# 🚀 Campus-Dash

A modern, streamlined dashboard designed to centralize essential information, services, and utilities for university students and staff.

### ✨ Features

* **Modular Architecture:** Built with React and TypeScript for scalability and maintainability.
* **Real-time Updates:** Designed to display user interactive interface with updates regarding items purchased and payment confirmation.
* **Intuitive Interface:** A clean, user-friendly design focused on quick access to items and products as per user requirements

---

### ⚙️ Tech Stack

This project is built using the following core technologies:

| Category | Technology |
| :--- | :--- |
| **Frontend** | React, TypeScript, Vite |
| **Language** | TypeScript (97.8%) |
| **Package Manager**| npm |

---

### 💻 Getting Started

Follow these steps to set up and run the project locally.

#### Prerequisites

* Node.js (LTS version recommended)
* npm or yarn

#### 1. Clone the repository

```bash
git clone [https://github.com/Shakya99/Campus-Dash.git](https://github.com/Shakya99/Campus-Dash.git)
cd Campus-Dash
