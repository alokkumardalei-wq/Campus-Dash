...
function App() {
  ...
  useEffect(() => {
    const initApp = async () => {
      // Check session
      const currentUser = await mockBackend.fetchCurrentUser();
    ...
    <div className="min-h-screen bg-transparent text-white selection:bg-primary selection:text-black font-sans relative">
      ...
      <div className="fixed bottom-4 right-4 z-50">
        <button
          ...
          className="bg-red-600 text-white px-4 py-2 rounded-full shadow-lg hover:bg-red-700 text-xs font-bold"
        >
          Seed DB
        </button>
      </div>
